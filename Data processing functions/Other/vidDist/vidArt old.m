...
% myprint      - scales and positions figure for printing
...
% line 291:  CheckStruct(vArt,defvArt)
% line 243:  Dist(filename)
% line 496:  GetIma()
% line 620:  getSubROI()
% line 648:  myprint(fh)
% line 692:  ProcWave(fn)
% line 822:  Save2XL(ddiam,DistHead)
% line 928:  Save2txt(vlog)
% line 1120: getMWave
% line 1148: linefit(data,acclevel,ord)
% line 1189: GenFilName(fn,fext,mode)
% line 1219: ECGtr=getRtop(diam)
% line 1263: ddiam=getMan_ECG(ddiam,mdiam,hsub)
% line 1347: res=getWaveStat(rtop,sdistr)
% line 1417: xt=getXT(segm,fps,xl)
% line 1438: set_Broi(ima) 
% line 1541: wpos=IniLP(ima)
% line 1645: wpos=wdet(ima,cblk,wall,disp)
% line 1726: [fh,hsub]=GenSubPlot(npl,fn,FOS)
% line 1766: frsel=Get_avi_Frame(ima,fn)
% line 1847: rprt=ClearXLsheet(xlsfn,xltab)
% line 1889: [wpos,imt]=getIMT(ima,wpos,imt,cblk) 
% line 1984: ipos=imt_ln(echoln,wp,cstr)
% line 2074: [wpos,IMTpos]=mod_MarkPos(wpos,IMTpos,cblk,cstr)
% line 2228: getPixScal()
% line 2282: limmess(str,dur)
% line 2305: [infotxt,abbr]=getInfo()
% 
% Most functions are embedded, so for compilation use:
%   mcc -m vidArt.m readVEVOBMode.m startup.m
...
vArt.version='Video Dist. V15.01';
...
vArt.dres=0.3;            % estimated depth US resolution (mm) within ROI
vArt.lres=1.3;          % estimated lateral US resolution (mm) within ROI
...
vArt.rproc=0;             % 0: normal analysis: 1:distension only; 2: all
...
while 2>1                                       % loop until program exit
    ...
    butt=listdlg('Liststring',...
        ...
        'Reprocess distension waveform only';...               % option 5
        'Reprocess (Edit wall position,distension, IMT)';...   % option 6
        ['General information (Version ',vArt.version,')'];... % option 7
    ...
    elseif (butt==5)||(butt==6)    % recall and re-evaluate processed data
        vArt.rproc=1;                % reconsider only distension waveform
        if butt==6
            ModSett();
            vArt.rproc=2;    % reprocess distension, wall position and IMT
        end
        ...
        if ferr==0
            ...
            for fnr=1:nrfiles
                ...
                if strfind(matfn,vArt.mfext)    % this is a proper matfile
                    ...
                    vArt=CheckStruct(vArt,defvArt);     % update structure
                    vArt.sgwind=defvArt.sgwind;      % and use new setting
                    vArt.rtthr=defvArt.rtthr;
                    [rpcont,sdistr,adiam]=ProcWave(ddistr); % wave process
                    if rpcont>0
                        if butt==6
    ...
    if butt==7             % provide general information about the program
...
while butt>1
    ...
    butt=listdlg('Liststring',...
        ...
        ['Font Size (currently ',num2str(vArt.FOS),')'];...     % option 2
    ...
    elseif butt==6               % set ultrasound depth resolution
        rep=char(inputdlg_mod(['Depth resolution in um (lateral res.',...
            ' 5 times larger)'],'US Resolution',1,...
            {sprintf('%d',1000*vArt.dres)}));
        if ~isempty(rep)
            vArt.dres=sscanf(rep,'%d')/1000;             % expressed in mm
            vArt.lres=5*vArt.dres;    % lateral resolution default 5 times
        end
    end
end
end
...
while rpcont<0
    ...
    xlabel(tstr);
    yadj=zeros(1,vArt.nfr);             % vertical position ROI over time
    sadj=0;
    ...
    for fr=1:vArt.nfr
        ...
        for wall=1:2
            for ln=1:vArt.nblk                              % skewed area!
                ...
                tmp(:,ln)=mean(im1(bp:bp+wrange,cblk(1,ln):cblk(3,ln)),2);
            end
            ...
            wpos(wall,:)=min(wref(wall,:)+2*vArt.wexpl,...
                max(wref(wall,:)-2*vArt.wexpl,wpos(wall,:)));
        end
        ddistr(:,fr)=wpos(2,:)-wpos(1,:);
        lcp=lcp-(lcp-(wpos(1,:)+wpos(2,:))/2)/2;%LPF lumen center position
        cshift=ceil((frbwind(4)/2-mean(lcp))/2);            % center shift
        if abs(cshift)>0&&(frbwind(2)+frbwind(4)-cshift<vArt.iDepth)
            frbwind(2)=frbwind(2)-cshift;
            wpos=wpos+cshift;
            wref=wref+cshift;
            nerr=nerr+1;                   % number of repositioned frames
            sadj=sadj+cshift;
        end  
        yadj(fr)=frbwind(2);
        wposT(:,:,fr)=wpos;
    ...
    set(dcm_obj,'UpdateFcn',{@myupdatefcn,rawlines,hsub,swp},...
        'DisplayStyle','datatip','Enable','on');
%     set(fh,'PaperPositionMode','auto')
%     print(fh,GenFilName(vArt.fn,'*.png',0),'-dpng','-r150')
    uiwait(fh);       % wait until "continue",skip or "repeat" is selected
    close all
    if rpcont>0     % if rpcont<0 -> repeat; elseif rpcont==0 -> skip file
        [rpcont,sdistr,adiam]=ProcWave(ddistr);          % wave processing
        if (rpcont>0)&&((vArt.rproc==0)||(vArt.rproc==2))
            frbwind=round(vArt.frbwind); % 1: continue, 0: skip; 1: repeat
            rima=zeros(frbwind(4)+1,frbwind(3)+1,vArt.necg,'uint8');
            for fr=1:vArt.necg
                frbwind(2)=round(yadj(vArt.rtop(fr)));
            ...
            [~, mname]=fileparts(vArt.fn);
                ...
                save(matfn,'vArt','cblk','rima','wposT','wposR','ddistr',...
                    'sdistr','adiam','imt','yadj','rpcont','wpos1');
...
function txt=myupdatefcn(~,event_obj,h,hsub,wpos)% Customize text tips
    set(h,'LineWidth',1)
    width = getappdata(event_obj.target, 'messages');
    txt   = sprintf('width (pixel): %d t/m %d',width(1),width(2));
    ...
    plot(width(1):width(2),wpos(1:2,width(1):width(2)),...
...
elseif strcmp(vArt.source,'*.bmode')       % it is a VEVO B-mode data file
    try
        ...
        [ima1,pixSize,ferr]=readVEVOBMode(vArt.fn,-1);   % read all frames
...
function myprint(fh)
% function scales/centers figure prior to printing. If figure fits within
% printwindow figure size will be retained, otherwise only the aspect 
% ratio is maintained! In the process all figure/object dimensions 
% are converted to normalized units to permit figure rescaling and 
% eventually converted back to their original units.
fobj=[fh; get(fh,'Children')];                   % main graphics objects
Nobj=length(fobj);
oldunit=cell(Nobj,1);
for ch=1:Nobj
    if isprop(fobj(ch), 'Units')
        oldunit{ch}=get(fobj(ch),'Units');
        set(fobj(ch),'Units','Normalized');% ensure objects are resizeable
    end
end
set(fh,'units','centimeters');                   % check size main figure
set(fh,'PaperUnits','centimeters');
fpos=get(fh,'Position');                   % retain old figure dimensions
set(fh,'units','normalized');                   % except for main figure
figasp=fpos(4)/fpos(3);                        % figure aspect ratio (y/x)
if fpos(4)>fpos(3)
    maxy=28;
    papwind=[1,1,18,maxy];
    set(fh,'PaperPositionMode','manual','PaperOrientation','portrait');
else                                       % go for landscape presentation
    maxy=18;
    papwind=[1,1,28,maxy];
    set(fh,'PaperPositionMode','manual','PaperOrientation','landscape');
end
papwind(1)=papwind(1)+max(0,(papwind(3)-fpos(3))/2);% center horizontally
papwind(3)=min(papwind(3),fpos(3));
papwind(4)=papwind(3)*figasp;
papwind(3:4)=papwind(3:4)*min(1,maxy/papwind(4));
papwind(2)=papwind(2)+max(0,(maxy-papwind(4))/2);    % center  vertically
set(fh,'PaperPosition',papwind);
print(fh);
for ch=1:Nobj
    if ~isempty(oldunit{ch})
        set(fobj(ch),'Units',oldunit{ch});   % restore figure/object units
    end
end
end
 

function [cont,sdistr,adiam]=ProcWave(ddistr)            % wave processing
% Smooths the curves,
...
function doWait(~,~)                            % evaluate menu selection
    ...
    if psel==2
        ...
        uiresume
    elseif psel==3   % modify/set manually positions R-top (end-doastole)
...
if (length(vArt.fn)>4)&&(strcmp(vArt.fn(2:4),'000'))
    ...
    while isempty(fn)
        fn=char(inputdlg(str, 'Name modification'));
...
txtcell(1:2,40)={'Ddiast.smn.tmn';num2str(vlog{3,10})};
txtcell(1:2,41)={'Ddiast.smn.tmed';num2str(vlog{4,10})};
txtcell(1:2,42)={'Ddiast.smn.tsd';num2str(vlog{5,10})};

txtcell(1:2,43)={'Dsyst.smn.tmn';num2str(vlog{3,14})};
txtcell(1:2,44)={'Dsyst.smn.tmed';num2str(vlog{4,14})};
txtcell(1:2,45)={'Dsyst.smn.tsd';num2str(vlog{5,14})};

txtcell(1:2,46)={'Ddist.smn.tmn';num2str(vlog{3,18})};
txtcell(1:2,47)={'Ddist.smn.tmed';num2str(vlog{4,18})};
txtcell(1:2,48)={'Ddist.smn.tsd';num2str(vlog{5,18})};

txtcell(1:2,49)={'Ddiast.smed.tmn';num2str(vlog{3,11})};
txtcell(1:2,50)={'Ddiast.smed.tmed';num2str(vlog{4,11})};
txtcell(1:2,51)={'Ddiast.smed.tsd';num2str(vlog{5,11})};

txtcell(1:2,52)={'Dsyst.smed.tmn';num2str(vlog{3,15})};
txtcell(1:2,53)={'Dsyst.smed.tmed';num2str(vlog{4,15})};
txtcell(1:2,54)={'Dsyst.smed.tsd';num2str(vlog{5,15})};

txtcell(1:2,55)={'Ddist.smed.tmn';num2str(vlog{3,19})};
txtcell(1:2,56)={'Ddist.smed.tmed';num2str(vlog{4,19})};
txtcell(1:2,57)={'Ddist.smed.tsd';num2str(vlog{5,19})};

txtcell(1:2,58)={'Ddiast.shom.tmn';num2str(vlog{3,12})};
txtcell(1:2,59)={'Ddiast.shom.tmed';num2str(vlog{4,12})};
txtcell(1:2,60)={'Ddiast.shom.tsd';num2str(vlog{5,12})};

txtcell(1:2,61)={'Dsyst.shom.tmn';num2str(vlog{3,16})};
txtcell(1:2,62)={'Ddsyst.shom.tmed';num2str(vlog{4,16})};
txtcell(1:2,63)={'Ddsyst.shom.tsd';num2str(vlog{5,16})};

txtcell(1:2,64)={'Ddist.shom.tmn';num2str(vlog{3,20})};
txtcell(1:2,65)={'Ddist.shom.tmed';num2str(vlog{4,20})};
txtcell(1:2,66)={'Ddist.shom.tsd';num2str(vlog{5,20})};

txtcell(1:2,67)={'Ddiast.A.tmn';num2str(vlog{3,13})};
txtcell(1:2,68)={'Ddiast.A.tmed';num2str(vlog{4,13})};
txtcell(1:2,69)={'Ddiast.A.tsd';num2str(vlog{5,13})};

txtcell(1:2,70)={'Dsyst.A.tmn';num2str(vlog{3,17})};
txtcell(1:2,71)={'Dsyst.A.tmed';num2str(vlog{4,17})};
txtcell(1:2,72)={'Dsyst.A.tsd';num2str(vlog{5,17})};

txtcell(1:2,73)={'Ddist.A.tmn';num2str(vlog{3,21})};
txtcell(1:2,74)={'Ddist.A.tmed';num2str(vlog{4,21})};
txtcell(1:2,75)={'Ddist.A.tsd';num2str(vlog{5,21})};

txtcell(1:2,76)={'rDist.smn.tmn';num2str(vlog{3,22})};
txtcell(1:2,77)={'rDist.smn.tmed';num2str(vlog{4,22})};
txtcell(1:2,78)={'rDist.smn.tsd';num2str(vlog{5,22})};

txtcell(1:2,79)={'rDist.smed.tmn';num2str(vlog{3,23})};
txtcell(1:2,80)={'rDist.smed.tmed';num2str(vlog{4,23})};
txtcell(1:2,81)={'rDist.smed.tsd';num2str(vlog{5,23})};

txtcell(1:2,82)={'rDist.shom.tmn';num2str(vlog{3,24})};
txtcell(1:2,83)={'rDist.shom.tmed';num2str(vlog{4,24})};
txtcell(1:2,84)={'rDist.shom.tsd';num2str(vlog{5,24})};

txtcell(1:2,85)={'rDist.A.tmn';num2str(vlog{3,25})};
txtcell(1:2,86)={'rDist.A.tmed';num2str(vlog{4,25})};
txtcell(1:2,87)={'rDist.A.tsd';num2str(vlog{5,25})};

txtcell(1:2,88)={'RT.smn.tmn';num2str(vlog{3,26})};
txtcell(1:2,89)={'RT.smn.tmed';num2str(vlog{4,26})};
txtcell(1:2,90)={'RT.smn.tsd';num2str(vlog{5,26})};

txtcell(1:2,91)={'RT.smed.tmn';num2str(vlog{3,27})};
txtcell(1:2,92)={'RT.smed.tmed';num2str(vlog{4,27})};
txtcell(1:2,93)={'RT.smed.tsd';num2str(vlog{5,27})};

txtcell(1:2,94)={'RT.shom.tmn';num2str(vlog{3,28})};
txtcell(1:2,95)={'RT.shom.tmed';num2str(vlog{4,28})};
txtcell(1:2,96)={'RT.shom.tsd';num2str(vlog{5,28})};

txtcell(1:2,97)={'RT.A.tmn';num2str(vlog{3,29})};
txtcell(1:2,98)={'RT.A.tmed';num2str(vlog{4,29})};
txtcell(1:2,99)={'RT.A.tsd';num2str(vlog{5,29})};

txtcell(1:2,100)={'IMT.smn.tmn';num2str(vlog{3,30})};
txtcell(1:2,101)={'IMT.smn.tmed';num2str(vlog{4,30})};
txtcell(1:2,102)={'IMT.smn.tsd';num2str(vlog{5,30})};

txtcell(1:2,103)={'IMT.smed.tmn';num2str(vlog{3,31})};
txtcell(1:2,104)={'IMT.smed.tmed';num2str(vlog{4,31})};
txtcell(1:2,105)={'IMT.smed.tsd';num2str(vlog{5,31})};

txtcell(1:2,106)={'IMT.shom.tmn';num2str(vlog{3,32})};
txtcell(1:2,107)={'IMT.shom.tmed';num2str(vlog{4,32})};
txtcell(1:2,108)={'IMT.shom.tsd';num2str(vlog{5,32})};

txtcell(1:2,109)={'IMT.A.tmn';num2str(vlog{3,33})};
txtcell(1:2,110)={'IMT.A.tmed';num2str(vlog{4,33})};
txtcell(1:2,111)={'IMT.A.tsd';num2str(vlog{5,33})};

txtcell(1:2,112)={'rIMT.smn.tmn';num2str(vlog{3,34})};
txtcell(1:2,113)={'rIMT.smn.tmed';num2str(vlog{4,34})};
txtcell(1:2,114)={'rIMT.smn.tsd';num2str(vlog{5,34})};

txtcell(1:2,115)={'rIMT.smed.tmn';num2str(vlog{3,35})};
txtcell(1:2,116)={'rIMT.smed.tmed';num2str(vlog{4,35})};
txtcell(1:2,117)={'rIMT.smed.tsd';num2str(vlog{5,35})};

txtcell(1:2,118)={'rIMT.shom.tmn';num2str(vlog{3,36})};
txtcell(1:2,119)={'rIMT.shom.tmed';num2str(vlog{4,36})};
txtcell(1:2,120)={'rIMT.shom.tsd';num2str(vlog{5,36})};

txtcell(1:2,121)={'rIMT.A.tmn';num2str(vlog{3,37})};
txtcell(1:2,122)={'rIMT.A.tmed';num2str(vlog{4,37})};
txtcell(1:2,123)={'rIMT.A.tsd';num2str(vlog{5,37})};

txtcell(1:2,124)={'IMT.max.tmn';num2str(vlog{3,38})};
txtcell(1:2,125)={'IMT.max.tmed';num2str(vlog{4,38})};
txtcell(1:2,126)={'IMT.max.tsd';num2str(vlog{5,38})};
...
for beat=1:ncycle                                % process segment signals
    ...
    for sig=1:nsig 
        ...
        value(sig,beat,7)=100*value(sig,beat,6)/value(sig,beat,1);%rel.imt
