function [outputMat] = simulateUniaxialViscoelasticBehaviour(timeV,lambdatV,lambdazV,modelFormulation,modelParameters,...
    reference_configuration,activeActivator,trigger)

%(xV_elastic,xV_visco,timeV,lambdatV,lambdazV,riV,hV,elastic,visco)
% tV needs to be a row vector, all other vectors should be columns
...
for iConstituent=1:elasticModel.nConstituents
    ...
    sigmatt_eV = sigmatt_rr_eV; % Lagrange multiplier to get actual circ. stress
    sigmazz_eV = sigmazz_rr_eV; % Lagrange multiplier to get actual axial stress
    sigmarr_eV = 0; % Lagrange multiplier to get actual radial stress
...
% PV = sigmatt_rr_ve_compV.*hV./(riV+hV/2)/mtN;
% % PV_compV = sigmatt_rr_ve_compV.*hV./(riV);
% fTV = sigmazz_rr_ve_compV.*(pi*hV.*(2*riV+hV))/gtN;

[PV,fTV] = stress2pressure_force(sigmatt_rr_ve_compV,sigmazz_rr_ve_compV,rmV,hV);

% ratio_circ = sigmatt_ve_compV_elastin./sigmatt_rr_ve_compV*100;
% ratio_axial = nan(length(ratio_circ),1);

% % Convert total axial force in tranducer force
% fT_compV = fV_compV - pi*riV.^2.*PV_compV;
...
% exportMat = [timeV(end-20:end)'-500,PV_compV(end-20:end)/mtN,2*riV(end-20:end),2*(riV(end-20:end)+hV(end-20:end)),fT_compV(end-20:end)/gtN,...
%     lambdatV(end-20:end),sigmatt_ve_compV(end-20:end),lambdazV(end-20:end),sigmazz_ve_compV(end-20:end),ratio_circ(end-20:end),ratio_axial(end-20:end)];
end