function [costV] = objectiveViscoelastic(parameterV_normalised,inputDataMatrixViscoelastic,elasticModel,viscousModel,n_dataPoints,weight_vector,test_list)
% Objective function for the optimisation of the model parameters of a
% visceoelastic passive model. The error is calculated using the
% normalised differences between modelled and experimental pressure and
% modelled and experimental transducer axial force.
% 
% Input parameters are normalised parameter vector (parameterV_normalised), the
% matrix of experimental data (inputDataMatrixViscoelastic), the passive elastic
% model formulation (elasticModel), the viscous model formulation
% (viscousModel), the cumulative number of datapoints in all experiments 
% (n_dataPoints), a vector of wieghts that allows to weigh differently different
% experiments, and a list of names of the experiment. 
% Note that parameterV_norm contains all the model parameters (i.e., the 
% concatenation of the passive elastic and viscous model parameters).

%% De-normalise elastic and viscous parameter vector and add imposed parameters
    parameterV_elastic_normalised = parameterV_normalised(1:elasticModel.nParam);
    parameterV_viscous_normalised = parameterV_normalised(elasticModel.nParam+1:end);
    
    parameterV_elastic_normalised=parameterV_elastic_normalised.*elasticModel.normV;
    ...
    for i=1:elasticModel.nParam_baseline
        if(isnan(elasticModel.imposedParam(i)))
            j=j+1;
            parameterV_elastic(i)=parameterV_elastic_normalised(j);
        elseif(isinf(elasticModel.imposedParam(i)))
            if(strcmp(elasticModel.modelName,'4-fiber families (not neo-Hookean)') || strcmp(elasticModel.modelName,'4-fiber families'))
                if(i==4 || i==7)
                    parameterV_elastic(i)=parameterV_elastic_normalised(2);
                elseif(i==6 || i==8)
                    parameterV_elastic(i)=parameterV_elastic_normalised(3);
                end
            end
        else
    ...
    parameterV_viscous = parameterV_viscous_normalised.*viscous_normV;

    % % NEEDS TO BE DELETED
    % xV_visco(4:6) = xV_visco(1:3);
    % % NEEDS TO BE DELETED
...
%% Initialise vectors
    ...
    lambdattV_final = zeros(n_dataPoints,1);
    lambdazzV_final = zeros(n_dataPoints,1);
    weightttV_final = zeros(n_dataPoints,1);
    weightzzV_final = zeros(n_dataPoints,1);

    ratio_exp = zeros(12,1);
    ratio_comp = zeros(12,1);
    err_slope = zeros(12,1);

%% Calculate modelled behaviour for each test
%     test_list = fieldnames(dataMat);   
    ...
    for i = 1:length(test_list) % number of test
        ...
        trigger_test = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,13);
    
        tV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,1)'; % time vector
        lambdatV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,8); % circumferential stretch
        lambdazV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,9); % axial stretch
        lambdarV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,10); % radial stretch
    
        riV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,4); % inner radius
        hV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,5); % wall thickness
    
        sigmatt_expV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,11); % experimental circumferential Cauchy stress
%         sigmazz_expV = dataMat.(test_list{i}).processed_data(:,12); % experimental axial Cauchy stress
    
        PV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,2); % experimental pressure
        fV = inputDataMatrixViscoelastic.(test_list{i}).processed_data(:,6); % experimental force
...
%             [P_eV,~] = stress2pressure_force(sigmatt_rr_eV,sigmazz_rr_eV,riV+hV/2,hV);
...
%             G_fun = viscous.fun(parameterV_viscous((iConstituent-1)*viscous.nParam+1:(iConstituent-1)*viscous.nParam+3),tM,0);
            locator = isnan(G_fun);
            G_fun(locator) = 0;
            ...
            % Viscoelastic Cauchy circumferential extra stress at the first
            % time point
...
%         [P_compV,~] = stress2pressure_force(sigmatt_rr_ve_compV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter),...
%             [],riV(locator_1:end)+hV(locator_1:end)/2,hV(locator_1:end));
%         P_compV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = P_compV;

%         sigmatt_expV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = sigmatt_expV(locator_1:end);
        P_expV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = PV(locator_1:end);
        lambdattV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = lambdatV(locator_1:end);
        
        % Weighting system to give the same fitting weight to quasi-static
        % pressure sweeps and quasi-static axial force sweeps
        weightttV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = weight_vector(i);
...
%             [~,fT_compV] = stress2pressure_force(sigmatt_rr_ve_compV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter),...
%                 sigmazz_rr_ve_compV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter),riV(locator_1:end)+hV(locator_1:end)/2,hV(locator_1:end));
...
%             fT_compV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = fT_compV;

            fT_expV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) =fV(locator_1:end);
            lambdazzV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = lambdazV(locator_1:end);
            
            % Weighting system to give the same fitting weight to quasi-static
            % pressure sweeps and quasi-static axial force sweeps
            weightzzV_final(1+counter:length(sigmatt_ve_compV(locator_1:end,1))+counter) = weight_vector(i);
            ...
            sigmatt_qs_exp_loading = sigmatt_qs_exp(3:max_qs-2);
            sigmatt_qs_exp_unloading = sigmatt_qs_exp(max_qs+2:end-3);
            sigmatt_qs_comp_loading = sigmatt_qs_comp(3:max_qs-2);
            sigmatt_qs_comp_unloading = sigmatt_qs_comp(max_qs+2:end-3);
            lambdatt_qs_loading = lambdatt_qs(3:max_qs-2);
            lambdatt_qs_unloading = lambdatt_qs(max_qs+2:end-3);
            ...
            lambdazzV_final = lambdazzV_final(1:end-length(sigmatt_ve_compV(locator_1:end,1)));
...
%         [sigmatt_rr_tf_temp,sigmazz_rr_tf_temp,~,~,~]=elastic.fun(parameterV_elastic,lambdatt_tf,lambdazz_tf,lambdarr_tf,[1,0]);
%         sigmatt_rr_tf = sigmatt_rr_tf+sigmatt_rr_tf_temp*1/(1+parameterV_viscous((iConstituent-1)*viscous.nParam+1)*log(parameterV_viscous((iConstituent-1)*viscous.nParam+3)/(parameterV_viscous((iConstituent-1)*viscous.nParam+2))));
%         sigmazz_rr_tf = sigmazz_rr_tf+sigmazz_rr_tf_temp*1/(1+parameterV_viscous((iConstituent-1)*viscous.nParam+1)*log(parameterV_viscous((iConstituent-1)*viscous.nParam+3)/(parameterV_viscous((iConstituent-1)*viscous.nParam+2))));
%     end

%     [P_tf_comp,fT_tf_comp] = stress2pressure_force(sigmatt_rr_tf,sigmazz_rr_tf,unloadedInnerRadius+unloadedWallThickness/2,unloadedWallThickness);

%     cost_tt = ([P_compV_final;P_tf_comp]*mtN - [P_expV_final;0])./mean(P_expV_final).*[weightttV_final; 1];
%     
%     cost_zz = ([fT_compV_final;fT_tf_comp]*gtN - [fT_expV_final;0])./mean(fT_expV_final).*[weightzzV_final; 1];
...
%     cost_zz = [(sigmazz_ve_compV_final-sigmazz_expV_final).*denom_zzV_final/mean(fT_expV_final).*weightzzV_final; sigmazz_rr_tf*pi*H.*(2*Ri+H)/mean(fT_expV_final)];
    ...
    costV = cat(1,cost_tt,cost_zz,err_slope); %cost function for optimization
