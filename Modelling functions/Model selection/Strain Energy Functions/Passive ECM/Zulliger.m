function [sigmatt_rr,sigmazz_rr, W, Ctttt, Czzzz]=Zulliger(parameterV,lambdatV,lambdazV,lambdarV,activator)
...
% Journal of Biomechanics, 40, pp.3061-3069, 2007.
%
...
sigmatt_rr=activator(1)*parameterV(1)*((lambdatV*parameterV(7)).^2-(lambdarV/parameterV(7)/parameterV(8)).^2).*(I1-3).^0.5+activator(2)*2*lambdatV.^2.*...
    sum((sigmatt_diagonal_fibre.*p_diagonal_fibre*0.001)')';
sigmazz_rr=activator(1)*parameterV(1)*((lambdazV*parameterV(8)).^2-(lambdarV/parameterV(7)/parameterV(8)).^2).*(I1-3).^0.5+activator(2)*2*lambdazV.^2.*...
    sum((sigmazz_diagonal_fibre.*p_diagonal_fibre*0.001)')';

% Calculate stored energy
W_diagonal_fibre=0.5*(parameterV(2)*(matA>0)+parameterV(3)*(matA<=0)).*matA.^2;
W=activator(1)*(parameterV(1)/2)*(I1-3)+activator(2)*sum((W_diagonal_fibre.*p_diagonal_fibre*0.001)');
